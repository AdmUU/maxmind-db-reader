...
class Reader
{
     ...
     *                                   from it
     */
    public function __construct(string $database, ?string $buff = null)
    {
    ...
    private function readNode(int $nodeNumber, int $index): int
    {
        ...
        switch ($this->metadata->recordSize) {
            case 24:
                $bytes = Util::read($this->fileHandle, $baseOffset + $index * 3, 3, $this->contentBuff);
            ...
            case 28:
                $bytes = Util::read($this->fileHandle, $baseOffset + 3 * $index, 4, $this->contentBuff);
                if ($index === 0) {
            ...
            case 32:
                $bytes = Util::read($this->fileHandle, $baseOffset + $index * 4, 4, $this->contentBuff);
                $rc = unpack('N', $bytes);
                if ($rc === false) {
                    throw new InvalidDatabaseException(
                        'Could not unpack the unsigned long of the node.'
